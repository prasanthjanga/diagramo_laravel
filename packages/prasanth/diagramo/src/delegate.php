...
//        exit();
    ...
    protected function getMultiple($tableName, $conditions = null, $orders = null, $start = null, $nr = null) {
        ...
        $className = ucfirst($tableName);//exit;
        // $className = 'user()';
        while ($row = $result->fetchArray()) {
            if ($tableName == 'user') {
                $object = new User;//exit();
            }elseif ($tableName == 'diagram') {
                $object = new Diagram;//exit();
            }elseif ($tableName == 'diagramdata') {
                $object = new Diagramdata;//exit();
            }else{
                $object = new $className;//exit();
            }
...
//        exit();
    ...
    public function diagramGetById($diagramId) {
        // echo 'this is the pageeeeeeeeee'.$diagramId;
        // exit;
        (DEBUG) ? $_SESSION['logs'][] = __CLASS__ .'{#}'. __FUNCTION__ ."{#}{#}". __LINE__ : '';
